...
@section('test_test')
   ...
   <div class="d-flex align-items-center justify-content-center pb-0 mb-0">
    <div class=" btn record_btn py-0 col-md-3">
        <a href="/amc/create" class="btn btn-outline-primary btn-outline-custom-primary mt-1 ml-0 mr-5 rounded-button px-5"><h6 class=""> + Add New Record</h6></a>
    </div>

    <div class="search-bar col-md-3">
        <div class="box">
            <div class="search-box">
                <form action="amc/search_data" method="GET">
            ...
            <div class="modal fade" id="budgetModal-{{ $record->year }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form id="budgetForm-{{ $record->year }}" class="budget-form" method="POST" enctype="multipart/form-data" action="/amc/budget_edit/{{ $budget->id }}">
                            @csrf
        ...
        @endforeach
            ...
            <div class="d-flex table_display justify-content-between">
                <div class="col-md-4 d-flex align-items-center">
                    ...
                    <a href="{{ route('amc.generatePdf', ['year' => $record->year, 'month' => $record->sub_category]) }}" class="btn btn-sm btn-dark d-inline rounded">Generate PDF</a>
                </div>
                        ...
                        <div class="modal fade" id="categoryBudgetModal-{{ $category->year }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form id="categoryBudgetForm-{{ $record->year }}" class="budget-form" method="POST" enctype="multipart/form-data" action="/amc/category_budget_edit/{{ $category->id }}">
                                        @csrf
                                        <div class="form-group modal-body">
                ...
                <tbody>
                    ...
                    @if (date("Y", strtotime($product->invoice_date)) == $record->year && $product->sub_category == $record->sub_category)
                            <tr class="">
                                ...
                                <td>
                                        <a href="/amc/{{ $product->id }}/show" class="">
                                ...
                                <td>
                                    ...
                                    @else
                                        <a href="amc/{{ $product->id }}/edit" class="btn btn-outline-info btn-sm rounded-button">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                    @endif
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.edit-budget').click(function() {
                ...
                $('#budgetForm-' + year).attr('action', '/amc/budget_edit/' + budgetId);
            });
        });
        ...
        $(document).ready(function() {
            $('.category-edit-budget').click(function() {
                ...
                var year = modalId.split('-')[1];
                $('#budgetId-' + year).val(budgetId);
                $('#categoryBudgetForm-' + year).attr('action', '/amc/category_budget_edit/' + budgetId);
            });
        });
        ...
        $('#rateModal').on('show.bs.modal', function (event) {
            ...
            var formAction = "/amc/" + productId + "/usd_rate";
        ...
        $('#approveModal').on('show.bs.modal', function (event) {
            ...
            var productId = button.data('product-id');
            $('#product_id').val(productId);
            var formAction = "/amc/" + productId + "/approve";
        ...
        $('#exampleModal').on('show.bs.modal', function (event) {
            ...
            var productId = button.data('product-id');
            $('#product_id').val(productId);
            var formAction = "/amc/" + productId + "/reject";
         ...
         $('#confirmDeleteModal').on('show.bs.modal', function (event) {
            ...
            form.attr('action', 'amc/' + productId + '/delete'); // Set the action attribute of the form
        });
