            ...
            <div class="col-sm-10">
              <div class="card mt-1 px-4 py-3 shadow rounded-year">
                <form action="/" method="" enctype="multipart/form-data">
                  ...
                  <div class="row pb-3">
                    <a href="/annual_capex" class="btn btn-outline-primary btn-outline-custom-primary rounded-year col-md-1"><i class="fa-solid fa-arrow-left"></i></a>
                ...
                <div class="form-group row">
                  ...
                  <div class="col-md-3">
                    ...
                    @if ($product->file)
                            @php
                                $fileUrl = asset('annuals/' . $product->file);
                                $fileExtension = pathinfo($fileUrl, PATHINFO_EXTENSION);
                            @endphp

                            @if (in_array($fileExtension, ['xlsx', 'docx', 'pdf', 'xls', 'csv']))
                                <p><b>File:</b> <a href="{{ $fileUrl }}" target="_blank" class="btn btn-outline-primary btn-outline-custom-primary rounded-year" style="padding: 0.2rem 0.5rem;">View File</a></p>
                            @else
                                <p><b>File:</b> No preview available. <a href="{{ $fileUrl }}" target="_blank">Download</a></p>
                            @endif
                        @else
                            <br>
                    ...
                    <div class="form-group row justify-content-center">
                      <div class="dropdown col-md-1 text-center">
                        ...
                        <select id="currency" name="currency" class=" btn btn-md btn-dark mb-1 rounded-year mt-2">
                            <option>{{ old('currency', $product->currency) }}</option>
                        </select>
                      </div>
                      <div class="col-md-2">
                ...
                <div class="form-group p-0">
                    <h6 class="font-weight-bold pb-1 text-center text-primary">Payment record details</h6>
                    <br>
                    <div class="form-group row justify-content-center">
                        <div class="col-md-3">
                            <label class="font-weight-bold">Record created by:</label>
                            <input type="text" name="tds" id="tds" class="form-control rounded-year" value="{{  $product->created_by }}"/>
                            @if($errors->has('tds'))
                                <span class="text-danger">{{ $errors->first('tds') }}</span>
                            @endif
                        </div>
                        <div class="col-md-3">
                            <label class="font-weight-bold">Record edited by:</label>
                            <input type="text" name="tds" id="tds" class="form-control rounded-year" value="{{ $product->edited_by }}"/>
                            @if($errors->has('tds'))
                                <span class="text-danger">{{ $errors->first('tds') }}</span>
                            @endif
                        </div>
                        <div class="col-md-3">
                            <label class="font-weight-bold">Payment made by:</label>
                            <input type="text" name="tds" id="tds" class="form-control rounded-year" value="{{ $product->approved_by }}"/>
                            @if($errors->has('tds'))
                                <span class="text-danger">{{ $errors->first('tds') }}</span>
                            @endif
                        </div>  
