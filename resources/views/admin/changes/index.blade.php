    ...
    <div class="container">
        <div class="container mt-6">
            <div class="row justify-content-end">
              ...
              <div class="col-auto">
                <form action="/admin/change/search_data" method="GET" class="form-inline">
        ...
        <div class="d-flex">
          ...
          <a href="{{ route('admin.change.generatePdf', ['year' => $record->year, 'month' => $record->month]) }}" class="btn btn-sm btn-dark d-inline">Generate PDF</a>
        </div>
              ...
              <tbody>
                  ...
                  @php
                      ...
                      $changes = $changes->sortByDesc('invoice_date');
                  @endphp

                  @foreach($changes as $change)
                      @if (date("Y", strtotime($change->invoice_date)) == $record->year && date("n", strtotime($change->invoice_date)) == $record->month)
                          <tr class="shadow">
                              <td>
                                <div class="container d-flex align-items-center">
                                  <div class="row justify-content-center align-items-center">
                                      <div class="col-4 text-center p-2">
                                          <div style="width: 10px; height: 10px; background-color: {{ $change->status_color }}; border-radius: 50%;">
                                          </div>
                                      </div>
                                  </div>
                                </div>
                              ...
                              <td>
                                <b>
                                  <a href="products/{{ $change->id }}/show" class="text-dark">
                                  {{ $change->invoice_no }}</a>
                                </b>
                              </td>
                              ...
                              <td> <b>{{ $change->currency }}</b> {{ $change->amount }}</td>
                              
                              <td>
                              
                                  <a href="/admin/change/{{ $change->id }}/show"
                                  class="btn btn-outline-info btn-sm shadow">View</a>

                              </td>
                              <td style="width: 175px;">
                                <form action="/change/{{ $change->id }}/approve" method="POST" enctype="multipart/form-data" style="display: inline-block;">
                                    @csrf
                                    <button type="submit" class="btn btn-outline-success btn-sm shadow">Approve</button>
                                </form>
                                <button type="button" class="btn btn-outline-danger btn-sm shadow" data-toggle="modal" data-target="#exampleModal" data-product-id="{{ $change->id }}">
                                  Reject
                                </button>
                              ...
                              <script>
                                  $('#exampleModal').on('show.bs.modal', function (event) {
                                      ...
                                      var formAction = "/change/" + productId + "/reject";
