    ...
    <div class="container">
        <div class="container mt-6">
            <div class="row justify-content-end">
              ...
              <div class="col-auto">
                <form action="/admin/annual/search_data" method="GET" class="form-inline">
        ...
        <div class="d-flex">
          ...
          <a href="{{ route('admin.annual.generatePdf', ['year' => $record->year, 'month' => $record->month]) }}" class="btn btn-sm btn-dark d-inline">Generate PDF</a>
        </div>
              ...
              <tbody>
                  ...
                  @php
                      ...
                      $annuals = $annuals->sortByDesc('invoice_date');
                  @endphp

                  @foreach($annuals as $annual)
                      @if (date("Y", strtotime($annual->invoice_date)) == $record->year && date("n", strtotime($annual->invoice_date)) == $record->month)
                          <tr class="shadow">
                              <td>
                                <div class="container d-flex align-items-center">
                                  <div class="row justify-content-center align-items-center">
                                      <div class="col-4 text-center p-2">
                                          <div style="width: 10px; height: 10px; background-color: {{ $annual->status_color }}; border-radius: 50%;">
                                          </div>
                                      </div>
                                  </div>
                                </div>
                              ...
                              <td>
                                <b>
                                  <a href="products/{{ $annual->id }}/show" class="text-dark">
                                  {{ $annual->invoice_no }}</a>
                                </b>
                              </td>

                              <td>{{ date('d/m/Y', strtotime($annual->invoice_date)) }}</td>

                              <td>{{ $annual->description }}</td>
                              
                              <td> <b>{{ $annual->currency }}</b> {{ $annual->amount }}</td>
                              
                              <td>
                              
                                  <a href="/admin/annual/{{ $annual->id }}/show"
                                  class="btn btn-outline-info btn-sm shadow">View</a>

                              </td>
                              <td style="width: 175px;">
                                <form action="/annual/{{ $annual->id }}/approve" method="POST" enctype="multipart/form-data" style="display: inline-block;">
                                    @csrf
                  
                                    <button type="submit" class="btn btn-outline-success btn-sm shadow">Approve</button>
                                </form>
                                <button type="button" class="btn btn-outline-danger btn-sm shadow" data-toggle="modal" data-target="#exampleModal" data-product-id="{{ $annual->id }}">
                                  Reject
                                </button>
                              ...
                              <script>
                                  $('#exampleModal').on('show.bs.modal', function (event) {
                                      ...
                                      var formAction = "/annual/" + productId + "/reject";
